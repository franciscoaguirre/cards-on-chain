...
mod cards {
    ...
    pub struct Cards {
        ...
        card_metadata: Mapping<TokenId, CardMetadata>,
        /// Owner to cards mapping for enumeration
        owner_cards: Mapping<Address, Vec<TokenId>>,
        /// Next available card ID
        next_card_id: TokenId,
    }
    ...
    impl Cards {
        ...
        pub fn new() -> Self {
            let mut contract = Self {
                ...
                card_metadata: Mapping::default(),
                owner_cards: Mapping::default(),
                next_card_id: 1,
            };
            
            contract.mint_starter_cards();
            contract
        }
        
        fn mint_starter_cards(&mut self) {
            ...
            for card in starter_cards {
                self.card_metadata.insert(card.id, &card);
        ...
        /// Mint a new card with metadata
        #[ink(message)]
        pub fn mint_card(&mut self, to: Address, metadata: CardMetadata) -> Result<TokenId, Error> {
            let card_id = self.next_card_id;
            let mut card_metadata = metadata;
            card_metadata.id = card_id;
            
            if card_metadata.cost > 10 || card_metadata.attack > 20 || card_metadata.health > 20 {
                return Err(Error::InvalidCardData);
            }
            
            self.card_metadata.insert(card_id, &card_metadata);
            self.add_token_to(&to, card_id)?;
            
            let mut owner_cards = self.owner_cards.get(&to).unwrap_or_default();
            owner_cards.push(card_id);
            self.owner_cards.insert(&to, &owner_cards);
        ...
        fn remove_token_from(
            ...
            let mut owner_cards = self.owner_cards.get(from).unwrap_or_default();
            if let Some(pos) = owner_cards.iter().position(|&x| x == id) {
                owner_cards.remove(pos);
                self.owner_cards.insert(from, &owner_cards);
            }
    ...
    impl CardDataProvider for Cards {
        #[ink(message)]
        fn get_card_metadata(&self, card_id: shared::CardId) -> Option<CardMetadata> {
            self.card_metadata.get(card_id)
        }
        ...
        #[ink(message)]
        fn cards_of_owner(&self, owner: Address) -> Vec<shared::CardId> {
            self.owner_cards.get(&owner).unwrap_or_default()
        }
    }
    ...
    mod tests {
        ...
        fn mint_works() {
            ...
            // Alice does not owns tokens initially.
            assert_eq!(cards.balance_of(accounts.alice), 0);
            // Create token Id 4 (starter cards use 1-3).
            assert_eq!(cards.mint(4), Ok(()));
            // Alice owns 1 token.
            assert_eq!(cards.balance_of(accounts.alice), 1);
        }

        #[ink::test]
        fn mint_card_with_metadata_works() {
            use shared::{CardMetadata, CardType, EffectType};
            ...
            let card_id = cards.mint_card(accounts.alice, metadata).unwrap();
            assert_eq!(card_id, 4); // Starter cards are 1-3
            assert_eq!(cards.owner_of(card_id), Some(accounts.alice));
            
            let stored_metadata = cards.get_card_metadata(card_id).unwrap();
            assert_eq!(stored_metadata.cost, 4);
            assert_eq!(stored_metadata.attack, 4);
            assert_eq!(stored_metadata.health, 5);
        }

        #[ink::test]
        fn starter_cards_exist() {
            let cards = Cards::new();
            
            // Check starter cards exist
            assert!(cards.card_exists(1));
            assert!(cards.card_exists(2));
            assert!(cards.card_exists(3));
            
            // Check metadata
            let card1 = cards.get_card_metadata(1).unwrap();
            assert_eq!(card1.cost, 1);
            assert_eq!(card1.attack, 1);
            assert_eq!(card1.health, 1);
        }
        ...
        fn mint_existing_should_fail() {
            ...
            let mut cards = Cards::new();
            // Create token Id 1.
            assert_eq!(cards.mint(1), Ok(()));
            // The first Transfer event takes place
            assert_eq!(1, ink::env::test::recorded_events().len());
            // Alice owns 1 token.
            assert_eq!(cards.balance_of(accounts.alice), 1);
            // Alice owns token Id 1.
            assert_eq!(cards.owner_of(1), Some(accounts.alice));
            // Cannot create  token Id if it exists.
            // Bob cannot own token Id 1.
            assert_eq!(cards.mint(1), Err(Error::TokenExists));
        }
        ...
        fn transfer_works() {
            ...
            assert_eq!(cards.balance_of(accounts.bob), 0);
            // The first Transfer event takes place
            assert_eq!(1, ink::env::test::recorded_events().len());
            // Alice transfers token 1 to Bob
            assert_eq!(cards.transfer(accounts.bob, 1), Ok(()));
            // The second Transfer event takes place
            assert_eq!(2, ink::env::test::recorded_events().len());
            // Bob owns token 1
            assert_eq!(cards.balance_of(accounts.bob), 1);
        }
        ...
        fn invalid_transfer_should_fail() {
            ...
            assert_eq!(cards.transfer(accounts.bob, 2), Err(Error::TokenNotFound));
            // Token Id 2 does not exists.
            assert_eq!(cards.owner_of(2), None);
            // Create token Id 2.
            assert_eq!(cards.mint(2), Ok(()));
            // Alice owns 1 token.
            assert_eq!(cards.balance_of(accounts.alice), 1);
            // Token Id 2 is owned by Alice.
            assert_eq!(cards.owner_of(2), Some(accounts.alice));
            // Set Bob as caller
            set_caller(accounts.bob);
            // Bob cannot transfer not owned tokens.
            assert_eq!(cards.transfer(accounts.eve, 2), Err(Error::NotApproved));
        }
        ...
        fn approved_transfer_works() {
            ...
            assert_eq!(cards.approve(accounts.bob, 1), Ok(()));
            // Set Bob as caller
            set_caller(accounts.bob);
            // Bob transfers token Id 1 from Alice to Eve.
            assert_eq!(
                cards.transfer_from(accounts.alice, accounts.eve, 1),
                Ok(())
            );
            // TokenId 3 is owned by Eve.
            assert_eq!(cards.owner_of(1), Some(accounts.eve));
            // Alice does not owns tokens.
            assert_eq!(cards.balance_of(accounts.alice), 0);
            // Bob does not owns tokens.
            assert_eq!(cards.balance_of(accounts.bob), 0);
            // Eve owns 1 token.
            assert_eq!(cards.balance_of(accounts.eve), 1);
        }
        ...
        fn approved_for_all_works() {
            ...
            assert_eq!(cards.mint(2), Ok(()));
            // Alice owns 2 tokens.
            assert_eq!(cards.balance_of(accounts.alice), 2);
            // Approve token Id 1 transfer for Bob on behalf of Alice.
            assert_eq!(cards.set_approval_for_all(accounts.bob, true), Ok(()));
            // Bob is an approved operator for Alice
            assert!(cards.is_approved_for_all(accounts.alice, accounts.bob));
            // Set Bob as caller
            set_caller(accounts.bob);
            // Bob transfers token Id 1 from Alice to Eve.
            assert_eq!(
            ...
            assert_eq!(
                cards.transfer_from(accounts.alice, accounts.eve, 2),
                Ok(())
            );
            // Bob does not own tokens.
            assert_eq!(cards.balance_of(accounts.bob), 0);
            // Eve owns 2 tokens.
            assert_eq!(cards.balance_of(accounts.eve), 2);
            // Remove operator approval for Bob on behalf of Alice.
            set_caller(accounts.alice);
            assert_eq!(cards.set_approval_for_all(accounts.bob, false), Ok(()));
        ...
        fn approve_nonexistent_token_should_fail() {
            ...
            assert_eq!(cards.approve(accounts.bob, 1), Err(Error::TokenNotFound));
        }
        ...
        fn not_approved_transfer_should_fail() {
            ...
            assert_eq!(cards.balance_of(accounts.eve), 0);
            // Set Eve as caller
            set_caller(accounts.eve);
            // Eve is not an approved operator by Alice.
            assert_eq!(
                cards.transfer_from(accounts.alice, accounts.frank, 1),
        ...
        fn burn_works() {
            ...
            assert_eq!(cards.owner_of(1), Some(accounts.alice));
            // Destroy token Id 1.
            assert_eq!(cards.burn(1), Ok(()));
            // Alice does not owns tokens.
            assert_eq!(cards.balance_of(accounts.alice), 0);
            // Token Id 1 does not exists
            assert_eq!(cards.owner_of(1), None);
        }
        ...
        fn burn_fails_token_not_found() {
            ...
            assert_eq!(cards.burn(1), Err(Error::TokenNotFound));
        }
        ...
        fn burn_fails_not_owner() {
            ...
            set_caller(accounts.eve);
            assert_eq!(cards.burn(1), Err(Error::NotOwner));
        }
        ...
        fn burn_clears_approval() {
            ...
            assert_eq!(
                cards.transfer_from(accounts.frank, accounts.bob, 1),
                Err(Error::NotApproved)
            );
        }
        ...
        fn transfer_fails_not_owner() {
            ...
            assert_eq!(cards.transfer(accounts.bob, 1), Err(Error::NotOwner));
        }
